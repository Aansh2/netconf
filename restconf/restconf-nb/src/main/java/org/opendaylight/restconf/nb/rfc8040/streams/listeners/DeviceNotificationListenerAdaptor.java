...
public final class DeviceNotificationListenerAdaptor extends AbstractNotificationListenerAdaptor
    ...
    private void reRegisterDeviceNotification(YangInstanceIdentifier path)
        ...
        Collection<? extends NotificationDefinition>
                notificationDefinitions =
                mountPoint.getService(DOMSchemaService.class)
                        .orElseThrow(() -> new RestconfDocumentedException("Device does not support notification",
        ...
        resetRegistration();
        listen(mountPoint.getService(DOMNotificationService.class)
                .orElseThrow(() -> new RestconfDocumentedException("Device does not support notification",
                        ErrorType.APPLICATION, ErrorTag.OPERATION_FAILED)), absolutes);
        getSubscribers().forEach(subscriber -> {
            if (subscriber.isConnected()) {
